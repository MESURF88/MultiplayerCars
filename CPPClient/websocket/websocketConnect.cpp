...
std::shared_ptr<WebsocketSession> WebsocketConn(net::io_context& ioc, ssl::context& ctx, std::string host, std::string port, std::string otp, std::function<void(const std::string&)> readcb)
{
    ...
    std::shared_ptr<WebsocketSession> session = std::make_shared<WebsocketSession>(ioc, ctx, readcb);
